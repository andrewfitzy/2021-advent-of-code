...
def process_file(filename, step_iterations, expected_result):
    """
    ...
    flashes = process_steps(step_iterations, octopuses)

    assert flashes == expected_result, 'Output is not as expected, expected ' + str(expected_result) + ' but got ' + str(flashes)

    print('RESULT: after ' + str(step_iterations) + ' iterations, ' + str(flashes) + ' pixels are lit for input ' + filename)


if __name__ == '__main__':
    process_file('input_example.txt', 100, 1656)

    # process_file('input_example.txt', 50, 3351)

    process_file('input.txt', 100, 1594)

    # process_file('input.txt', 50, 16383)
