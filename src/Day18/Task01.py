...
def solve(file_content):
    previous_answer = file_content[0]
    for index in range(1, len(file_content)):
        next_sum = add(previous_answer, file_content[index])
        next_sum = reduce(next_sum)
        previous_answer = next_sum
    return calculate_magnitude(previous_answer)
